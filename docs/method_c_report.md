...
반면 레이블이 없는 데이터는 795,315행이 존재한다. 이 중 동일 장비·부품으로 필터링해도 71,180행이 남는다 — 레이블 데이터의 약 9배다. **만약 unlabeled 데이터에서 불량 후보를 신뢰성 있게 선별할 수 있다면, 불량 학습 샘플을 크게 늘려 분류기 성능을 개선할 수 있다.** 이것이 방법 C의 동기다.
...
이 과정에서 가장 중요한 질문은 **"pseudo-label이 실제 공정 불량과 얼마나 유사한가"** 다. 이것이 충족되지 않으면 어떤 분류기를 써도 개선이 없다. 방법 C 전체는 이 질문에 대한 답을 찾아가는 과정이다.
...
이 섹션은 `08_pseudo_labeling.ipynb`의 Section 3~4(PCA/UMAP)와 `08a~08i` EDA 노트북을 토대로 한다.
...
**주목할 결과:**
- 불량 71개 모두 PC1·PC2 기준으로 양품의 범위 안에 혼재 (100% 겹침)
- → **불량은 선형적으로 분리되지 않는다.** 단순 이상탐지로 잡기 어렵다는 것을 PCA가 먼저 보여준다.
...
UMAP은 비선형 구조를 포착한다. 결과의 의미:

1. **불량이 단일 클러스터로 뭉쳐 있지 않다** — 퍼짐 비율 0.75x는 불량이 여러 패턴으로 분산됨을 뜻한다. 단일 이상 패턴이 없으니 클러스터링 기반 탐지가 힘들다.
2. **Unlabeled가 labeled와 UMAP 공간에서 크게 겹친다** — 필터링이 유효했다는 증거. 두 데이터셋이 동일한 공정 분포에서 나왔다.
3. **불량 62개(87%) 주변에 unlabeled 이웃이 존재** — kNN 방식으로 불량 후보를 찾을 수 있다는 단서. 이것이 전략 D(kNN from Labeled Defects)의 근거가 된다.

### 3.3 EDA 노트북(08a~08i)의 역할

각 EDA 노트북은 이상탐지 기법 한 가지를 독립적으로 심층 분석한다. 단순히 ROC 숫자를 보는 것이 아니라:
- PCA/UMAP에 이상 점수를 overlay해 어느 영역의 샘플이 높은 이상 점수를 받는지 확인
- 피처별 이상 점수 상관도 → 어떤 피처가 이 기법의 "신호원"인지
- 병렬 좌표계 → 상위 이상 샘플과 실제 불량의 피처 패턴 비교
- KS 검정 → 양품/불량 이상 점수 분포가 통계적으로 다른지 검증
...
이 섹션이 방법 C 전체에서 가장 중요한 발견이다. 반드시 이해해야 한다.
...
RPM=0, 배럴온도≈0°C — 이것은 명백하게 **기계가 멈춰 있는 상태**다. Elliptic Envelope가 unlabeled 데이터에서 "이상"으로 분류한 것들이 공정 불량이 아니라 기계가 꺼진 상태의 레코드였다.
...
QDA는 클래스별 공분산 행렬로 결정 경계를 정의한다. 진짜 불량(온도≈280°C, RPM≈258)과 pseudo-불량(온도≈0°C, RPM≈0)을 같은 "불량" 클래스로 묶으면, 불량 클래스의 공분산 행렬이 두 개의 완전히 다른 분포의 혼합이 된다. 결정 경계가 완전히 왜곡되어 ROC -0.11이라는 붕괴 수준 하락이 발생했다.
...
- `sc_vis`: unlabeled 전체 분포를 기준으로 정규화 → PCA로 두 데이터셋의 공간 관계를 시각화할 때 unlabeled의 원래 분포 기준이 적합하다.
- `sc_sel`: labeled 분포를 기준으로 정규화 → 전략 선택 (어떤 unlabeled 샘플이 labeled 불량과 유사한가?)과 CV 학습 모두 labeled 공간에서 수행해야 일관성이 있다.

만약 `sc_vis`를 전략 선택에 쓰면, labeled와 unlabeled의 공간 기준이 달라진다. 예를 들어 unlabeled 71K에 기계정지가 44% 있으면 RPM의 평균이 낮아지고 분산이 커진다. 이 스케일러로 labeled를 변환하면 labeled 양품이 "높은 RPM" 방향으로 치우쳐 보이게 되어 전략 선택이 왜곡된다.
...
이 함수는 pseudo-defect들의 "집단적 이탈 방향"이 labeled 불량의 방향과 얼마나 일치하는지를 정량화한다. 두 지표:

- **dir_match**: 25개 피처 중 불량과 같은 방향으로 이탈한 피처의 비율 (50%=무작위)
...
Self-training은 DR=0.89% 극단 불균형에서 구조적으로 작동하기 어렵다. 이것은 구현 문제가 아니라 방법론 자체의 한계다. 불균형이 심할수록 분류기가 불량에 높은 확신도를 부여하는 경우가 극히 드물기 때문이다.
...
*이 보고서는 `08_pseudo_labeling.ipynb`, `08_method_c_v2.ipynb`, `08a~08i_eda.ipynb`의 실행 결과를 토대로 작성됐다. 수치는 모두 실제 실행 출력값이며 재현 가능하다. (SEED=42, fold_*.npy 고정)*
