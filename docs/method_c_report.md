...
가이드북은 DNN [128,64,32]를 최종 권장 모델로 제시했다(재현 결과: ROC=0.9468, PR=0.4655). 방법 C 결과와 직접 비교하면 다음과 같다.

| 모델 | ROC-AUC | PR-AUC | 비고 |
|---|---|---|---|
| 가이드북 DNN [128,64,32] (재현) | 0.9468 | 0.4655 | 비교 기준 |
| 방법 C 전략 D (5-fold CV, RF) | **0.9531** | 0.4572 | 정식 CV 결과 |
| 방법 C 전략 D, n=50 (ablation) | **0.9619** | **0.4826** | n 탐색 후 선택 |

5-fold CV 기준으로는 ROC +0.006 앞서지만 PR -0.008 뒤진다. n=50 ablation 결과는 양쪽 다 앞서지만, [50~2000] 중 최적을 골라 비교한 수치다 — 재현 가능하지만 정식 CV와 동등하게 인정받기는 어렵다.

**Final report 프레이밍 주의사항**

"방법 C가 가이드북을 이겼다"고 주장하면 PR 수치로 반박당할 수 있다. 대신 다음 각도가 더 방어하기 쉽다.

가이드북 §2.2.2는 pseudo-labeling을 "향후 방향"으로 언급만 하고 실제 구현은 없다. 방법 C는 이를 직접 구현하면서 두 가지 구체적인 문제를 발견하고 수정했다 — 기계정지 오염(44%)과 teacher RF 정보 누수. 성능 수치의 절대 우위보다 **"가이드북이 제안한 방향을 실제로 검증하고, 거기서 발생하는 실패 원인을 분석한 팀"** 이라는 포지션이 originality 측면에서 더 강하다.
