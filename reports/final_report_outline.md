...
**들어갈 내용:**  
사출성형 불량 예측의 산업적 중요성(자동차 앞유리 사이드 몰딩, PassOrFail 라벨 기준 불량률 0.89%)을 서술한다. 가이드북 §2.3의 "AE→SVM→DNN 순서 학습 후 best 1개 선택" 흐름이 각 처리 단계의 독립적 기여도를 측정하지 않는다는 점을 비판하고, 우리의 4-Phase Ablation이 차별점(originality)임을 명확히 한다. 강의 14개 알고리즘을 모두 체계적으로 시연하는 구조를 연구 목적으로 제시한다.
...
**들어갈 내용:**  
4-Phase Ablation의 전체 구조를 설명한다. 각 Phase는 독립 변수 1개(전처리 방식 / 모델 패밀리 / 차원축소 방법 / 메타 학습기)를 변경하고 나머지를 고정하여 성능 기여도를 분리 측정한다. 평가 지표(주: ROC-AUC + PR-AUC, 보조: F1, Precision@Recall=0.5/0.7/0.9), Stratified 5-fold CV(fold_0~4.npy 공유), 데이터 누수 방지(SMOTE/Scaler를 fold 내부에서 fit) 방침을 명시한다.
...
차원축소 Ablation(PCA 4종 vs 트리 중요도 3종) 결과: PCA는 성능 저하, TreeTop-15(0.9380)가 full(0.9343)을 소폭 초과함을 보고한다. 앙상블·NN 결과: MLP[256,128,64]+Dropout=0.3이 ROC-AUC 0.9497, PR-AUC 0.4710으로 전 Phase 최고치 달성. 가이드북 DNN 재현(0.9468/0.4655) 대비 우리 MLP가 소폭 우위임을 비교 표에 제시한다.
...
**들어갈 내용:**  
"어느 단계가 성능을 가장 끌어올렸는가?"를 Phase별 기여도 정량화로 답한다. 가장 큰 기여: Phase 4 비선형 모델 전환(PR-AUC +0.119). 두 번째: Phase 3 모델 패밀리 내 최적화(PR-AUC +0.084). 불균형 처리(Phase 2)는 점수 향상보다 분산 안정화 역할. Stacking 반례가 "항상 앙상블이 낫다"는 통념을 반박함을 논의한다. 가이드북의 "AE→SVM→DNN 단일 best" 방식이 QDA 같은 숨겨진 강자를 놓치는 위험을 정리한다.
