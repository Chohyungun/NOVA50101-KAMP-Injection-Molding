...
**CN7 vs RG3 분포:** KS 검정 결과 24개 변수 중 22개(91.7%)에서 유의미한 분포 차이(p < 0.05)가 나왔다. 두 제품군을 통합 분석하면 분포 왜곡 위험이 있으므로, 이 텀프에서는 labeled_data.csv 단독 분석을 기본으로 채택했다.
...
가이드북은 어느 전처리 단계가 성능에 기여했는지, 불균형 처리가 얼마나 중요한지, SVM이 다른 선형 모델 대비 실제로 우월한지를 분리해 측정하지 않는다.

### 우리의 접근: 단계별 기여도 분리 측정

각 처리 단계를 독립적으로 변경하고 나머지를 고정해 **"어느 단계가 성능을 좌우했는가"**를 정량화한다.
...
이 구조가 가이드북의 "AE→SVM→DNN 순서 선택" 방식과 차별되는 지점이며, 각 단계의 성능 기여도를 분리 측정하는 것이 이 텀프의 originality 핵심이다.
...
Phase 3 최고 성능: **QDA(reg=0.01)** — 가이드북이 "SVM best"라고 주장한 결과를 정량적으로 반박한다.
...
PCA는 이 데이터에서 차원 감소 시 성능이 떨어졌다. RF 중요도 기반 Top-15는 전체 25개 피처 대비 소폭 우위를 보이며, 차원축소 ablation 전체가 가이드북에 없는 분석이다.
...
Precision≥0.99로 운용할 때 MLP가 불량 71건 중 약 23건을 탐지할 수 있다(재현율 32.4%).
...
**핵심 발견:** 가이드북이 "SVM best"라고 주장한 모델은 PR-AUC 기준 6개 선형 모델 중 최하위(SVM-RBF 0.0890)다. 가이드북이 시도조차 하지 않은 QDA가 PR-AUC 0.3526으로 Phase 3 전체 1위를 기록했다. 이 역설이 단계별 ablation의 originality를 정량적으로 뒷받침한다.
...
단계별 기여도를 보면 아래 네 가지 결론이 나온다.

**Phase 4(비선형 모델 전환)가 가장 큰 기여 단계다.** 선형/생성 모델에서 앙상블·MLP로 바꾸면서 PR-AUC가 +0.119 올랐다. 사출성형 공정 변수들 사이에 비선형 상호작용이 중요하다는 뜻이다.

**Phase 3(모델 패밀리 내 최적화)이 두 번째다.** QDA는 가이드북이 제시하지 않은 모델임에도 선형 패밀리 최고 성능을 냈다. 체계적 탐색 없이 단일 모델을 선택한 가이드북 접근법의 한계가 수치로 드러난다.

**Phase 2(불균형 처리)는 안정화 효과가 핵심이다.** SMOTE는 ROC-AUC 절대값을 높이기보다 PR-AUC 분산을 절반으로 줄여(std 0.1145→0.0554) 결과의 신뢰성을 높였다. 극단적 불균형(0.89%) 환경에서 안정성 확보가 단순 점수 향상만큼 중요하다.

**Phase 5는 반례다.** "스태킹은 항상 단일 모델을 이긴다"는 통념과 달리, 기반 모델의 다양성이 부족하면 단일 최고 모델(MLP)에 미달할 수 있다. 이 반례 자체가 OOF(폴드 외 예측) 방식의 한계를 보여주는 실증 사례다.
...
`labeled_data.csv`는 사출성형 사이클당 1행의 집계 통계로 구성된다. Phase 5에서 구현한 1D-CNN은 변수를 임의 순서의 1D 시퀀스로 처리(DeepInsight 방식)했으며, 실제 시계열 의존성을 모델링한 것이 아니다. 인접 사이클 간의 공정 드리프트(drift) 감지나 슬라이딩 윈도우 기반 시계열 분류는 충분히 시도하지 못했다.

향후 과제: 인접 K개 사이클을 묶는 슬라이딩 윈도우(K=10 기준 권장) 기반 입력을 구성하면, 강의 범위 내 CNN 구조(Conv1d → MaxPool → FC)를 유지하면서 시계열 의존성을 반영할 수 있다. 이동 평균(rolling mean K=5) 피처를 집계 변수에 추가하는 단순한 방법도 적용 가능하다.

**6.2 CN7/RG3 분리 분석 미완**

KS 검정 결과 두 제품군의 변수 분포가 22/24개 변수에서 유의하게 달랐음에도(p < 0.05), Phase 3~5는 labeled_data 단독(CN7+RG3 혼합) 분석을 기본으로 유지했다. 제품별 분리 모델링 또는 product flag를 피처로 추가하는 ablation을 계획했으나(decisions.md D-006, D-004) 실제로는 시도하지 않았다.

향후 과제: CN7 전용 모델과 RG3 전용 모델을 각각 5-fold CV로 학습해 혼합 모델과 ROC-AUC·PR-AUC를 비교한다. labeled_data_CN7_processed.csv와 labeled_data_RG3_processed.csv가 이미 존재하므로 즉시 착수 가능하다.

**6.3 불량 원인(Reason 컬럼) 99.1% 결측**

PassOrFail이 'N'(불량)인 71건 중 불량 원인(Reason 컬럼)이 기록된 건수가 거의 없다. 불량 원인을 피처로 쓰거나 원인별 분류 모델을 구성하는 것이 현실적으로 불가능한 상태다. KAMP 데이터 자체의 수집 프로세스 한계이며, 산업 현장에서 불량 원인 추적 체계 구축의 필요성을 시사한다.

**6.4 산업 현장 운용 임계값 설정의 어려움**

Precision≥0.99 운용점에서 MLP의 Recall은 32.4%에 불과하다. 불량 71건 중 약 48건이 탐지되지 않는데, 이는 데이터의 극단적 불균형(0.89%)과 양성 표본 절대 수 부족(71건)에서 비롯된다. 더 많은 불량 데이터 없이는 높은 Recall과 Precision을 동시에 달성하기 어렵다. "어느 수준의 Recall을 감수할 것인가"에 대한 도메인 전문가의 판단이 필요하다.

**6.5 가이드북 AE(Autoencoder) 재현 미완성**

가이드북 §2.3의 원본 흐름은 AE(Autoencoder)로 비지도 특성 추출 후 SVM/DNN을 학습하는 방식이다. AE는 강의 범위 밖 알고리즘(AGENT_INSTRUCTIONS §5.6에서 금지)이므로, 이 텀프에서는 AE 단계 없이 supervised 학습만 수행했다. 가이드북과의 완전한 1:1 비교가 AE 단계에서 불완전하다.
...
각 Phase 완료 시 팀 내 결과 리뷰 회의 기록을 남기고, GitHub commit 이력으로 코드 기여도를 확인할 수 있게 유지한다. decisions.md에는 주요 기술 결정 사항을 날짜·근거와 함께 지속적으로 기록한다.
